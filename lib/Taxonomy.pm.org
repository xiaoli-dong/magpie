...
sub taxid2sysname{
  ...
  return $self->{'taxid2sysname'}->{$data};
}
...
#	return $taxids[0];
 #   }
	...
	my ($parent_taxid) = $parent =~ /^\d+$/ ? $parent : $self->{'name2taxid'}->{$parent};
...
#print STDERR "parent=$parent\t$parent_taxid\n";
	...
	for my $taxid (@taxids){
		...
		#if($lineage_taxid == 1760){
		
		#print STDERR "taxid= $taxid, 1760 is here and lineage_distance = $lineage_distance **********************************\n";
		#}
		
		#print STDERR "$parent_taxid == $lineage_taxid\n";
	...
	if(not defined $shortest_distance or $lineage_distance < $shortest_distance){
	    ...
	    #print STDERR "&&&&&&&&&&&&&&& $taxid; $shortest_distance = $lineage_distance;\n";
...
sub updateTaxonomyIndices{
  ...
  unlink "$species_index_path.taxid2sysname"
    or (warn "Cannot delete old $species_index_path.taxid2sysname: $!" and return undef) if -e "$species_index_path.taxid2sysname";
  unlink "$species_index_path.name2taxid"
    or (warn "Cannot delete old $species_index_path.name2taxid: $!" and return undef) if -e "$species_index_path.name2taxid";
